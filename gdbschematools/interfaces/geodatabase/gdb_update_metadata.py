...
from ..converter import compare_comma_strings
        ...
        if domain_desc:
            if domain_desc.description != domain.description:
                arcpy.management.AlterDomain(
                    gdb_desc.catalogPath, domain.name, new_domain_description=domain.description
                )
...
def fds_update_metadata(gdb_struct: "Geodatabase", gdb_desc):
    ...
    for feature_dataset in feature_datasets:
        feature_dataset_name = (
            f"{gdb_desc.connectionProperties.database}.{feature_dataset.schema}.{feature_dataset.name}"
            if is_enterprise
            else feature_dataset.name
        )
        ...
        if fds_desc:
            ...
            if feature_dataset.meta_summary != fds_desc_meta_summary:
                gdb_metadata.update_metadata(fds_desc.catalogPath, "summary", feature_dataset.meta_summary)
        else:
...
def ds_update_metadata(gdb_struct: "Geodatabase", gdb_desc):
    ...
    for dataset in datasets:
        dataset_name = (
            f"{gdb_desc.connectionProperties.database}.{dataset.schema}.{dataset.name}"
            if is_enterprise
            else dataset.name
        )
        ...
        if dt_desc:
            ...
            if dataset.meta_summary != dt_desc_meta_summary:
                gdb_metadata.update_metadata(dt_desc.catalogPath, "summary", dataset.meta_summary)

            dt_desc_meta_description = gdb_metadata.get_dataset_description(dt_desc)
            if dataset.meta_description != dt_desc_meta_description:
                gdb_metadata.update_metadata(dt_desc.catalogPath, "description", dataset.meta_description)

            dt_desc_meta_tags = gdb_metadata.get_dataset_tags(dt_desc)
            if not compare_comma_strings(dataset.meta_tags, dt_desc_meta_tags):
                gdb_metadata.update_metadata(dt_desc.catalogPath, "tags", dataset.meta_tags)
            ...
            if domain.test_value(converted_code):
                ...
                if domain_meta_summary != domain_desc_meta_summary:
                    gdb_metadata.update_edomain_metadata(
                        dataset_desc.catalogPath, field_name, domain, code, domain_meta_summary
                    )
    else:
        ...
        for code in dataset_subtypes:
            if code in subtypes_desc:
                ...
                if dataset_subtypes[code].meta_summary != code_desc_meta_summary:
                    gdb_metadata.update_edomain_metadata(
                        path_to_ds, subtypes_field_name, dataset.subtype, code, dataset_subtypes[code].meta_summary
                    )
            else:
